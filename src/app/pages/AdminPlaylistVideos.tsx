...
export default function AdminPlaylistVideos() {
  ...
  async function handleAdd() {
    if (!itemId) return;
    const { errors } = await createVideo({
      title: 'New video',
      itemId,
      sortOrder: videos.length,
    });
    if (errors) {
      alert('Failed to add video: ' + errors.map((e) => e.message).join(', '));
      return;
    }
    refresh();
  }
  ...
  async function handleDelete(id: string) {
    ...
    const { errors } = await deleteVideo(id);
    if (errors) {
      alert('Failed to delete video: ' + errors.map((e) => e.message).join(', '));
      return;
    }
    refresh();
  }
  ...
  async function handleSave(video: any) {
    ...
    if (errors) {
      alert('Failed to save video: ' + errors.map((e) => e.message).join(', '));
    }
  }
