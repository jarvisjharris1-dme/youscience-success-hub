...
export default function AdminTaxonomy() {
  ...
  async function handleAddProduct() {
    ...
    if (errors) {
      alert('Failed to add product: ' + errors.map((e) => e.message).join(', '));
      return;
    }
    setNewProductTitle('');
    refreshProducts();
  ...
  async function handleDeleteProduct(id: string, title: string) {
    if (!confirm(`Delete product "${title}" and all its sections? Articles inside won't be deleted automatically — move or remove them first.`))
      return;
    const { errors } = await deleteProduct(id);
    if (errors) {
      alert('Failed to delete product: ' + errors.map((e) => e.message).join(', '));
      return;
    }
    refreshProducts();
  }
  ...
  async function handleDeleteCategory(productId: string, categoryId: string, title: string) {
    ...
    const { errors } = await deleteCategory(categoryId);
    if (errors) {
